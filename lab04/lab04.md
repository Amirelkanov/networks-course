...
### Пролог
Я написал прокси-сервер для проксирования веб-страниц c кэшированием и фильтрацией доменов/URLов. Используемые библиотеки приложил в `requirements.txt`. Немалая часть кода взята из предыдущей лабы, так что флаги тут похожие:
```
usage: main.py [-h] [--server_port SERVER_PORT] [--concurrency_level CONCURRENCY_LEVEL] [--log_level {DEBUG,INFO,WARNING,ERROR,CRITICAL}]
               [--log_file LOG_FILE]

proxy server for lab04
...
```
Логи во время демонстрации всех пунктов я приложу в файле proxy.log
...
Для проверки того, прокси объект в кеше или нет, необходимо использовать условный GET
...
#### Демонстрация работы
В постмане видно не будет, поэтому я просто приложу логи:
```
2025-03-29 03:03:33 - INFO - Proxying GET request to gaia.cs.umass.edu:80/wireshark-labs/HTTP-wireshark-file1.html
2025-03-29 03:03:35 - INFO - Serving https://gaia.cs.umass.edu/wireshark-labs/HTTP-wireshark-file1.html from cache (304 Not Modified)
```
