...
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - Заголовок `User-Agent` предоставляет строку с несколькими характеристиками:
     - Общая метка, которая обозначает, что мой браузер совместим с указанным
     - Платформа, на которой работает браузер
   
   P.s. У меня показывает несколько браузеров. Полагаю, это сделано для сайтов, которые отслеживают, какой браузер посещает страницу, и предоставляют некоторые функции только в том случае, если браузер IE/Chrome/Mozilla и т.п. 
...
5. Когда HTML-файл, который вы извлекаете, последний раз модифицировался на сервере?
   - Last-Modified: `Sat, 15 Feb 2025 06:59:01 GMT`
6. Сколько байтов контента возвращается вашему браузеру?
   - Content-Length: `128`

![1 task](images/image.png)
...
3. Теперь проверьте содержимое второго HTTP-запроса GET (из вашего браузера на сторону
   ...
   - Да. Заголовок-вопрос If-Modified-Since означает: `Не изменилась ли страница после даты, полученной в Last-Modified?`. Как раз дата, полученная в Last-Modified, следует за этим заголовоком.
...
1. Сколько HTTP GET запросов было отправлено вашим браузером? На какие интернет-адреса были отправлены эти GET-запросы?
   - 3 запроса
   - Адреса были следующими:
     - Сама страница: 128.119.245.12
     - Логотип Pearson: 128.119.245.12
     - Обложка книги: 178.79.137.164
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - Полагаю, параллельно, т.к. браузер, не дожидаясь ответа от GET-запроса на загрузку логотипа, отправил следующий GET-запрос на загрузку обложки
...
2. Когда ваш браузер отправляет сообщение HTTP GET во второй раз, какое новое поле включается в сообщение HTTP GET?
   - `Authorization`, включающий в себя `Credentials`, которые wireshark умеет показывать. Видимо, поэтому https и нужен...

![Task 5](images/image5.png)