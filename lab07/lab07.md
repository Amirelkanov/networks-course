...
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
   ...
   - Время получения подтверждений этих же сегментов в секундах (относительно начала захвата пакетов):
     ...
     - `6.051179`
     - ![](images/17.png)
   - А можете в фидбеке уточнить: я правильно понимаю, что тут что-то типа выборочного повторения со скользящим окном? Т.к. мало того, что почти все времена одинаковые, так еще и ACK умный, полагаю: он иногда как будто комбинирует несколько ACK в один, поэтому у меня и ACK-пакетов меньше
...
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   - Насчет времени: 
     - Ну, можно смотреть на разницу времен передачи данных: `время пакета 194` - `время пакета 51` = `0.34447с`
   ...
   - Ну давайте считать, что нас только полезный объем интересует:
     - $\frac{152138}{0.34447} ~ 4.4 * 10^5$ байт/с
